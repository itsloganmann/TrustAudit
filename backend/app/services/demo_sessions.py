...
def _anonymize(invoices: List[Dict]) -> List[Dict]:
    """Deterministically rename vendors to ``Vendor A/B/C...`` and strip
    every other PII column the webhook might have stuffed into the feed
    entry.
    ...
    Phase 0 hardening (round 2): also strip ``gstin`` and the real
    ``invoice_number`` so the unauthenticated REST endpoint
    ``GET /api/live/invoices`` cannot ever leak them. We synthesize
    safe public-display equivalents:
      * gstin → "•••• masked ••••"
      * invoice_number → first 3 chars + "•••" suffix (so the row
        can still be visually distinguished without exposing the full
        number)
    """
    ...
    for inv in invoices:
        ...
        # Mask GSTIN. PENDING placeholder stays as-is so the UI can
        # still render its "missing" state — only real values are masked.
        raw_gstin = str(copy.get("gstin") or "")
        if raw_gstin and raw_gstin != "PENDING":
            copy["gstin"] = "•••• masked ••••"

        # Mask the invoice number unless it's an obvious system-generated
        # placeholder ("WA-..." auto-generated by the webhook).
        raw_inv_no = str(copy.get("invoice_number") or "")
        if raw_inv_no and not raw_inv_no.startswith("WA-"):
            head = raw_inv_no[:3] if len(raw_inv_no) > 3 else raw_inv_no
            copy["invoice_number"] = f"{head}•••"
